...
class particle{
	...
	*/
	...
	//particle(int _pid); 
	particle(int _pid, bool decay_taus = true);
    private:
...
double normalize(vector<double> & branching_fraction){
	//add up the branching fractions so far and identify a branchign fraction that will make them all add up to 1. 
	double sum = 0.;
	for(auto ibr = branching_fraction.begin();ibr!=branching_fraction.end();ibr++) sum+= *ibr;
	return 1.0 - sum;
}

void safe_normalise(vector<double> & branching_fraction, double guess, double tollorence = 0.01){
	//appends a branching fraciton to branching_fraction that will normalize the vector. 
	//taking in a guess, and requries that the normalization be close to the guess to within a tollerence. 
	//default tollorence is 2%
	//Also tests for negative norm. . 
	//If the normalization is bad, it complains and quits. 
    double ideal_norm = normalize(branching_fraction);
    if(ideal_norm <= 0.){
	cout<<"Error! normalization ("<< ideal_norm <<") is <= zero. Guess was "<< guess << ". Go fix it!" << endl;
	std::terminate();
    }
    else if(fabs(ideal_norm-guess)/guess > tollorence){
	cout<<"Error! normalization ("<< ideal_norm <<") is sigificantly different than guess "<<guess<<". Go fix it!"<<endl;
	std::terminate();
    }
    branching_fraction.push_back(ideal_norm);
}//end safe_normalise
...
/*
bool stages_are_equal(stage* A, stage* B){
	//returns true iff A = B up to permutations. 
...
particle::particle(int _pid, bool decay_taus):pid(_pid){
...
pIDmap* makemap(bool decay_taus = true){
	...
	array<int,39> charged = {1, 4, 5, 605, 2305, 2505, 11, 6011, 2311, 2411, 25011,511, 6511, 23511, 24511, 25511, 513, 6513, 23513, 24513, 25513, 13, 6013, 2313, 2413, 25013, 2513, 15, 6015, 2315, 2415, 2515, 25015, 6, 24, 624, 2524, 55, 66};
	...
	for(unsigned int i=0;i<charged.size();i++){
		out->set(charged[i],  new particle( charged[i], decay_taus)); //delete1
		out->set(-charged[i], new particle(-charged[i], decay_taus)); //delete1
	}
	return out;
}//end makemap
...
particle* mapfetch(int pid){
	...
	//This is a performance critical function. 
    static pIDmap* pmap = makemap(DECAY_TAUS); //only done once at startup. 
...
void particle::setTau(int pid,bool decay_taus){
	...
	safe_normalise(branching_fraction,0.6476);
	...
	safe_normalise(branching_fraction, 0.1530);//4 decay products allowed, can fully model tau to hhhnu
	...
	safe_normalise(branching_fraction, 0.1530);//5h nu subsumed in 3h nu
	...
	array<int,LEN_D_ARR> hhhnu = {4,i_sign*1,i_sign*1, -i_sign*1, 12}; //fully modeled. 
	daughters.push_back(hhhnu);
	safe_normalise(branching_fraction, 0.15203);
...
void particle::setW(int pid){
	...
	//This represents the width for the decay of the W boson into a charged particle with momentum below detectability, p< 200 MeV.
	array<int,LEN_D_ARR> invisible = {2,12,12};
	daughters.push_back(invisible);
	branching_fraction.push_back(0.0140);
	//lepton decays
	array<int,LEN_D_ARR> enu = {2,i_sign*(prefix + 11),12};
	daughters.push_back(enu);
	branching_fraction.push_back(0.1071);
	array<int,LEN_D_ARR> mnu = {2,i_sign*(prefix + 13),12};
	daughters.push_back(mnu);
	branching_fraction.push_back(0.1063);
	array<int,LEN_D_ARR> tnu = {2,i_sign*(prefix + 15),12};
	daughters.push_back(tnu);
	branching_fraction.push_back(0.1138);

	//much of W's hadron decay is to charm
	array<int,LEN_D_ARR> cq = {2,-i_sign*4,i_sign*1};
	daughters.push_back(cq);
	branching_fraction.push_back(0.334);
	//rest is hadrons
	array<int,LEN_D_ARR> qq = {2,1,-1};
	daughters.push_back(qq);
	safe_normalise(branching_fraction, 0.3248);
...
//			#endif
	...
	branching_fraction.push_back(0.2000);//nunu
	daughters.push_back(bb);
	branching_fraction.push_back(0.15156);//bb
	daughters.push_back(cc);
	branching_fraction.push_back(0.1203);//cc
	daughters.push_back(ee);
	branching_fraction.push_back(0.03363);//ee
	daughters.push_back(mm);
	branching_fraction.push_back(0.03366);//mm
	daughters.push_back(tt);
	branching_fraction.push_back(0.03370);//tau tau
//	daughters.push_back(ggg);
//	branching_fraction.push_back(0.0110);//ggg 
	daughters.push_back(qq);
	safe_normalise(branching_fraction, 0.42715);

	nbranchings = daughters.size();
}//setZ
...
void particle::setH(){
	...
	daughters.push_back(bb);
	safe_normalise(branching_fraction, 0.569);
